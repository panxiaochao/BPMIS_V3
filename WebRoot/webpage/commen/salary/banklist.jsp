...
var addFun = function() {
	var dialog = parent.ext.modalDialog({
		title : '添加银行卡信息',
		url : 'webpage/commen/salary/bankadd.jsp',
		buttons : [{
			text : '&nbsp;添加&nbsp;',
			handler : function() {
...
var editFun = function(id) {	
	var dialog = parent.ext.modalDialog({
		title : '编辑类别信息',
		url : 'salary/getPojo_Bank.do?id='+id,
		buttons : [ {
			text : '&nbsp;编辑&nbsp;',
			handler : function() {
				dialog.find('iframe').get(0).contentWindow.submitForm(dialog, datagrid, parent.$);
	...
	if (rows.length > 0){
		$.messager.confirm('询问', '您是否要删除？', function(b) {
			if (b) {
				...
				$.post('salary/delbank.do', {id : ids.join(';')}, function(result) {
					if (result.success) {
...
var banktype = function(value,rowData,rowIndex) {	
	return rowData.bankvalue+"["+value+"]";
}
  ...
  <table id="tt" style=" border-bottom:none;">
    <thead>
      <tr>
        ...
        <th data-options="field:'bankname',width:120">银行卡名称</th>
